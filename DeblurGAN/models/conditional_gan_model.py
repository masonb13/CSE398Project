...
from util.metrics import SSIM1
...
class ConditionalGAN(BaseModel):
	...
	def backward_G(self):
		...
		#------------------------------------------------------------------------------------------------
		#self.loss_G = ssim(np.squeeze(self.real_A.detach().cpu().numpy().astype(np.float32)),np.squeeze(self.fake_B.detach().cpu().numpy().astype(np.float32)),full=True,channel_axis=0,data_range=1)[0]


# Compute SSIM loss
		self.loss_G = 1 - ssim(self.real_A,self.fake_B)  
